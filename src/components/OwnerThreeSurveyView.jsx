...
class OwnerThreeSurveyView extends React.Component {
  state = {
    survey: [],
    owner: []
  }
  ...
  componentWillMount(){
    axios.get('http://localhost:8000/surveys/3.json').then(response => this.setState({survey: response.data}))
    axios.get('http://localhost:8000/owners/3.json').then(response => this.setState({owner: response.data}))
  }
            ...
            <div className="section">
              <div className="card-panel grey darken-3 z-depth-5">
                ...
                <div className='row center top-pad'>
                  ...
                  <a href="/owner/3" className="btn-large waves-effect waves-light orange ">Back to Dashboard</a>
                </div>
                </div>
              </div>
            </div>
...
export default OwnerThreeSurveyView
